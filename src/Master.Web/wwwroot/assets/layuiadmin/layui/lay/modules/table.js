...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
	...
	table.exportFile = function (id, data, type) {
		...
		var buildTableTemplate = function (dataTitle, dataMain) {
			...
			layui.each(dataTitle, function (i, v) {
				if (v == "项目名称") {
					tpl += '<th width="520" style="font-size:18px" align="center">' + v + '</th>';
				} else if (v == "奖项大类") {
					tpl += '<th width="200" style="font-size:18px" align="center">' + v + '</th>';
				}
				else {
					tpl += '<th width="160" style="font-size:18px" align="center">' + v + '</th>';
				}
				
			});
			tpl +='</thead><tbody>';
			layui.each(dataMain, function (i, v) {
				tpl += '<tr>';
				var rowData = v.split(',');
				layui.each(rowData, function (j, v2) {
					console.log(v2);
					var val = v2;
					...
					tpl += '<td style="font-size:14px">' + val + '</td>';
				})
				tpl += '</tr>';
			})
			tpl += '</tbody>';
			//console.log(tpl);
			return format(template, { table: tpl, worksheet:config.title });
		};
		...
		alink.href = 'data:' + textType + ';charset=utf-8,\ufeff' + encodeURIComponent(function () {
			...
			return type == 'csv' ? (dataTitle.join(',') + '\r\n' + dataMain.join('\r\n')) : buildTableTemplate(dataTitle, dataMain);
