...
abp.event.on('abp.signalr.connected', function () { // Register for connect event
    //console.log('invoke sendAllMessage');
    //连接成功后获取未读消息
...
layui.use('layim', function () {
    ...
    /*Hub事件*/
    abp.signalr.hubs.common.on('getClientMessage', function (message) { // Register for incoming messages
        console.log(message);
        if (message.cid > 0) {
            //向服务器发送已读
            abp.signalr.hubs.common.invoke('setClientMessageReaded', message.cid);
        }
    ...
    layim.config({
        //初始化接口
        init: {
            url: '/api/services/app/LayIM/GetInitData'
            , data: {}
        }
        //查看群员接口
        , members: {
            url: '/api/services/app/LayIM/GetMember'
            , data: {}
        }

        , uploadImage: {
            url: '/File/LayIMUpload' //（返回的数据格式见下文）
        }
        , uploadFile: {
            url: '/File/LayIMUpload' //（返回的数据格式见下文）
        }
        ...
        ,copyright:true
        , msgbox: '/layim/msgbox' //消息盒子页面地址，若不开启，剔除该项即可
    ...
    layim.on('sendMessage', function (data) {
        ...
        abp.signalr.hubs.common.invoke('sendClientMessage', data)
            .then(function () {
                layer.msg("发送成功");
            });;
